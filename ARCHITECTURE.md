...
> **Restored 2026-06-10** from git history (last committed at `63b38c8`; the
> file was dropped in merge `7a8ff2b`). Known drift since the snapshot:
> `PipController` moved out of `js/app.js` into `js/pip.js` (lazy-loaded on
> first Pop-out); the three main scripts load with `defer`; the Pause/Hush
> control-bar and PiP buttons were retired (markup commented out —
> `TTSCoordinator.hush` itself still exists); a session-UX pass added the
> follow-latest lock, GoAway countdown, and in-app confirms. Line numbers and
> file sizes throughout are stale; section structure and invariants still hold.
> Passthrough specifics that changed 2026-06-10: sources now mix via
> `PtRing::mixFrom` (per-source offsets that auto-rebase on dislocation instead
> of dropping forever), dead sinks/sources are restarted on reconfigure with
> warnings in the `ready` event, and the browser's `CompanionPassthrough`
> waits for `ready` before `configure()` resolves and auto-reconnects after an
> unexpected close.
